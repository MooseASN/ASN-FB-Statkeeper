...
class JumbotronScheduleItem(BaseModel):
    ...
    source_url: str  # share code URL or PrestoSports XML URL
    start_time: str  # ISO timestamp
    end_time: Optional[str] = None  # ISO timestamp
    label: Optional[str] = None  # Optional name for the slot
...
@api_router.get("/jumbotron/configs/{config_id}")
...
@api_router.put("/jumbotron/configs/{config_id}")
...
@api_router.delete("/jumbotron/configs/{config_id}")
...
@api_router.get("/jumbotron/embed/{embed_code}")
async def get_jumbotron_embed_data(embed_code: str):
    ...
    # Find currently active schedule item based on time
    ...
    # If no scheduled item is active, use the first one as fallback
...
@api_router.post("/jumbotron/parse-prestosports")
async def parse_prestosports_xml(url: str = Body(..., embed=True)):
    ...
    try:
        async with httpx.AsyncClient(timeout=30.0) as client:
            ...
            # Try to extract game data from PrestoSports format
            game_data = {
                "source_type": "prestosports",
                "source_url": url,
                "sport": "basketball",  # Default, can be detected from URL
                "home_team_name": "",
                "away_team_name": "",
                "home_score": 0,
                "away_score": 0,
            ...
            # PrestoSports format varies, so we try multiple approaches
            ...
            # Try to get period/quarter info
...
@api_router.get("/jumbotron/user-games")
