...
- **Jumbotron Mode - venue scoreboard displays with scheduling and multiple layouts**
...
- **Montserrat Bold Font** - All jumbotron displays now use Montserrat Bold for professional broadcast look
- **Multiple Displays per Event** - Create different sized outputs for same tournament:
  - HD Overhead (1920×1080) - Full stats display
  - Scorers Table Wide (1920×400) - Stat leaders + center scoreboard
  - Minimal Score (1280×200) - Score only display
- **Three Layout Types:**
  - **Full Stats** - Complete player stats table with team headers
  - **Scorers Table** - 3 stat leaders per team on sides, team logos/names/scores in center
  - **Minimal** - Just team names, logos, and large scores
- **Layout Auto-Detection** - Automatically selects layout based on aspect ratio:
  - Aspect ratio > 3 → Scorers table
  - Aspect ratio > 2.5 → Minimal
  - Otherwise → Full stats
- **URL Parameter Support** - Force layout with `?layout=full|scorers|minimal`
- **Updated Backend Models:**
  - Added `JumbotronDisplay` model with id, name, width, height, layout
  - Configs now support `displays[]` array for multiple outputs
- **Improved Space Utilization:**
  - Full layout fills entire screen with no dead space
  - Tables use `tableLayout: fixed` for consistent column widths
  - Stats rows scale to available height

**Feature: Basketball Roster Sorting**
- On-floor players (checked) always appear at TOP of roster list
- Players sorted by jersey number within each group
- Substitutions immediately reorder the list

**Bug Fix: Demo Mode CORS Issue**
- Added `withCredentials: false` to all demo loader components
