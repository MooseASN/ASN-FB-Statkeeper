...
const BaseballDiamond = ({ bases, fieldingPositions }) => {
  const positions = {
    ...
    first: { top: '55%', left: '75%' },
    second: { top: '35%', left: '50%' },
    third: { top: '55%', left: '25%' },
  ...
  return (
    <div className="relative w-full aspect-square max-w-md mx-auto">
      {/* Outfield grass */}
      <div className="absolute inset-0 bg-green-800 rounded-t-full" />
      ...
      <svg className="absolute inset-0 w-full h-full" viewBox="0 0 100 100">
        {/* Diamond outline */}
        <path 
          d="M 50 85 L 75 55 L 50 25 L 25 55 Z" 
          fill="none" 
          stroke="white" 
          strokeWidth="0.5"
        />
        {/* Home plate */}
        <polygon points="50,85 47,82 47,80 53,80 53,82" fill="white" />
        {/* First base */}
        <rect x="73" y="53" width="4" height="4" fill={bases?.first ? "#22c55e" : "white"} transform="rotate(45 75 55)" />
        {/* Second base */}
        <rect x="48" y="23" width="4" height="4" fill={bases?.second ? "#22c55e" : "white"} transform="rotate(45 50 25)" />
        {/* Third base */}
        <rect x="23" y="53" width="4" height="4" fill={bases?.third ? "#22c55e" : "white"} transform="rotate(45 25 55)" />
        {/* Pitcher's mound */}
      ...
      {Object.entries(positions).map(([pos, coords]) => (
        <div 
          key={pos}
          className="absolute transform -translate-x-1/2 -translate-y-1/2 bg-zinc-800/90 px-2 py-1 rounded text-xs text-white whitespace-nowrap"
          style={{ top: coords.top, left: coords.left }}
        >
          {fieldingPositions?.[pos] ? (
            <>#{fieldingPositions[pos].number} {fieldingPositions[pos].name}</>
          ) : (
            <span className="text-zinc-500">{pos.toUpperCase()}</span>
          )}
        </div>
      ))}
    </div>
