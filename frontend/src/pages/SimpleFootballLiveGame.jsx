...
export default function SimpleFootballLiveGame({ demoMode = false, initialDemoData = null }) {
  ...
  const startSackWorkflow = () => { setActiveWorkflow('sack'); setWorkflowStep(1); setWorkflowData({}); };
  ...
  const handleRushTackler = (tackler) => {
    ...
    addPlay({
      type: 'run',
      carrier: rusher.player_number,
      yards: yards,
      tackler: tackler?.player_number || null,
      result: yards > 0 ? 'gain' : yards < 0 ? 'loss' : 'no_gain',
      description: `#${rusher.player_number} ${rusher.player_name} rush for ${yards} yds${tackler ? ` (tackled by #${tackler.player_number})` : ''}`
    });
    closeWorkflow();
  };
  ...
  return (
    <div className="h-screen bg-black text-white flex flex-col overflow-hidden" data-testid="simple-football-page">
      ...
      <div className="bg-zinc-900 border-b border-zinc-800 px-4 py-2 flex items-center justify-between shrink-0">
        <Button variant="ghost" size="sm" onClick={() => navigate(demoMode ? "/" : "/dashboard/football")} className="text-zinc-400" data-testid="back-btn">
