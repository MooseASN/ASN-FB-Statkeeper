...
export default function FootballLiveGame({ user, onLogout, demoMode = false, initialDemoData = null }) {
  ...
  const {
    ...
    advanceDown: hookAdvanceDown,
    changePossession: hookChangePossession,
    flipPossession: hookFlipPossession,
    spotBall: hookSpotBall,
    setDownAndDistance,
    advanceQuarter: hookAdvanceQuarter,
    addPlayToDrive: hookAddPlayToDrive,
    endDrive: hookEndDrive,
    initializeFromState,
    getStateForSave,
    getYardLineDisplay
  } = driveState;
  ...
  const [editPlayerTeam, setEditPlayerTeam] = useState(null); // 'home' or 'away'
      ...
      const updateRoster = (roster) => 
        roster.map(p => 
          p.id === editingPlayer.id 
            ? { ...p, number: editPlayerData.number, name: editPlayerData.name }
            : p
        );
      ...
      <header className="bg-zinc-900 border-b border-zinc-800 px-4 py-2">
        <div className="flex items-center justify-between">
          <Button
            variant="ghost"
            size="sm"
            onClick={() => navigate(demoMode ? "/" : "/dashboard/football")}
